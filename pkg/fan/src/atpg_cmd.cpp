...
ReadPatCmd::ReadPatCmd(const std::string &name, FanMgr *fanMgr_A, FanMgr *fanMgr_B) : Cmd(name)
{
	fanMgr_A_ = fanMgr_A;
	fanMgr_B_ = fanMgr_B;
...
bool ReadPatCmd::exec(const std::vector<std::string> &argv)
{
	...
	delete fanMgr_A_->pcoll;  delete fanMgr_B_->pcoll;
	...
	if (!patBlder_A->read(optMgr_.getParsedArg(0).c_str(), verbose) || !patBlder_B->read(optMgr_.getParsedArg(0).c_str(), verbose))
	{
		std::cerr << "**ERROR ReadPatCmd()::exec(): pattern builder error\n";
		delete fanMgr_A_->pcoll;  delete fanMgr_B_->pcoll;
		delete patBlder_A;  delete patBlder_B;
		fanMgr_A_->pcoll = NULL;  fanMgr_B_->pcoll = NULL;
...
AddFaultCmd::AddFaultCmd(const std::string &name, FanMgr *fanMgr_A, FanMgr *fanMgr_B) : Cmd(name)
{
	fanMgr_A_ = fanMgr_A;
	fanMgr_B_ = fanMgr_B;
	optMgr_.setName(name);
...
ReportFaultCmd::ReportFaultCmd(const std::string &name, FanMgr *fanMgr_A, FanMgr *fanMgr_B) : Cmd(name)
{
	fanMgr_A_ = fanMgr_A;
	fanMgr_B_ = fanMgr_B;
	optMgr_.setName(name);
...
bool ReportFaultCmd::exec(const std::vector<std::string> &argv)
{
	...
	std::cout << "#    number of faults: " << fanMgr_A_->fListExtract->faultsInCircuit_.size()+fanMgr_B_->fListExtract->faultsInCircuit_.size();
	std::cout << "\n";
...
ReportCircuitCmd::ReportCircuitCmd(const std::string &name, FanMgr *fanMgr_A, FanMgr *fanMgr_B) : Cmd(name)
{
	fanMgr_A_ = fanMgr_A;
	fanMgr_B_ = fanMgr_B;
	optMgr_.setName(name);
...
RunFaultSimCmd::RunFaultSimCmd(const std::string &name, FanMgr *fanMgr_A, FanMgr *fanMgr_B) : Cmd(name)
{
	fanMgr_A_ = fanMgr_A;
	fanMgr_B_ = fanMgr_B;
...
bool RunFaultSimCmd::exec(const std::vector<std::string> &argv)
{
	...
	if (!fanMgr_A_->pcoll || !fanMgr_B_->pcoll)
	{
	...
	if (!fanMgr_A_->sim || !fanMgr_B_->sim)
	{
